...
const weather = (latitude, longitude, callback) => {
    ...
    request({url, json: true}, (error, {body} = {}) => {
        if (error) {
            ...
            callback(undefined, {
                forecast: current.weather_descriptions[0] + ". Current temperatue is " + current.temperature + " degress with " + current.precip + " percent chances of rain with humidity of " + current.humidity + "%."
            })
        }
    })
}
